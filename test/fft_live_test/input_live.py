<<<<<<< HEAD:test_code/fft_live_test/input_live.py
import pyaudio
import wave
...
class Fourier:
    ...
    def fourier(self, f): # 人間の可聴域は20~20,000Hz
        ...
        F_ = np.concatenate([np.zeros(self.scale), F, np.zeros(self.scale)])
        # 周波数分布を定数倍
        # zero_ = np.zeros([self.scale-1, _*2])
        # F_ = np.broadcast_to(F, (self.scale,  _*2)).reshape(-1, order='F') # 定数倍したときの隙間は同じ値で補完
        # F_ = np.concatenate([np.array([F]), zero_]).reshape(-1, order='F') # 定数倍したときの隙間は0で補完
        # リサンプリング
        # t = np.arange(0, len(F))
        # f_linear = interpolate.interp1d(t, F, kind='cubic')
        # t = np.arange(0, len(F)-1, 1/self.scale)
        # F_ = f_linear(t)
        # リサイズ
        # inv_F_ = np.resize(f_, int((len(f_)*self.scale)))
        # inv_F_ = np.delete(np.reshape(inv_F_, [2, -1], order='F'), 1, axis=0).reshape(-1)
...
if __name__=="__main__":
    ...
    # freq, F, binv_F = f.fourier(frames)
    output.write(frames)
>>>>>>> 539d7b0c1a348a012d6824d53c2d42870f691c9f:test/fft_live_test/input_live.py
    audio.exit()