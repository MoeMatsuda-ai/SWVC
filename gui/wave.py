...
def dibox_graph(command):
    ...
    lb_one = tk.Radiobutton(
        frame_list, text="sound wave", value=False, font=font_list, variable=var_list
    )
    lb_two = tk.Radiobutton(
        frame_list,
        text="sound wave and frequency",
        value=True,
        font=font_list,
        variable=var_list,
    )
    ...
    entry_scale = tk.Entry(
        frame_scale, textvariable=var_scale, borderwidth=5, font=font_def
    )
    ...
    button_exec = tk.Button(
        ...
        borderwidth=5,
        text="Record",
        command=lambda: command.plot_graph(
            scale=int(var_scale.get()), freq=int(var_list.get())
        ),
    )
