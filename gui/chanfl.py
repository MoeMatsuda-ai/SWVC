...
def dibox_freq(command):
    ...
    lb_mult = tk.Radiobutton(
        frame_list, text="音の高さ", value="mult", font=font_list, variable=var_list
    )
    lb_ps = tk.Radiobutton(
        frame_list, text="しゅるい", value="ps", font=font_list, variable=var_list
    )
    ...
    var_rb = StringVar()
    lb_mult_high = tk.Radiobutton(
        frame_mult, text="高くする", value="high", font=font_list, variable=var_rb
    )
    lb_mult_low = tk.Radiobutton(
        frame_mult, text="ひくくする", value="low", font=font_list, variable=var_rb
    )
    ...
    sc_scale_low = ttk.Scale(
        frame_scale_low,
        variable=var_scale_low,
    ...
    sc_scale_high = ttk.Scale(
        frame_scale_high,
        variable=var_scale_high,
        orient=tk.HORIZONTAL,
        length=200,
        from_=100,
        to=1000,
    ...
    var_ps = StringVar()
    lb_syou = tk.Radiobutton(
        frame_ps, text="はんにん", value=-25, font=font_list, variable=var_ps
    )
    lb_han = tk.Radiobutton(
        frame_ps, text="しょうげんしゃ", value=30, font=font_list, variable=var_ps
    )
    lb_utyu = tk.Radiobutton(
        frame_ps, text="きかい音", value=300, font=font_list, variable=var_ps
    )
    lb_suzu = tk.Radiobutton(
        frame_ps, text="すず虫", value=500, font=font_list, variable=var_ps
    )
    ...
    button_exec = tk.Button(
        ...
        borderwidth=5,
        text="ろくおん",
        command=lambda: command.transform(
            tofile_origin=int(var_save.get()),
            tofile_chan=int(var_save_ts.get()),
            type=var_list.get(),
            ps_scale=int(var_ps.get()),
            mult_scale=mult_select(
                var_rb.get(), var_scale_low.get(), var_scale_high.get()
            ),
